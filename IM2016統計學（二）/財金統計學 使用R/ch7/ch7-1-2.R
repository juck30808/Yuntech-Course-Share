...
y = data[,2]
x = data[,1]
abline(lm(y~x),lwd=2) # 畫出迴歸線
points(x[1],y[2],pch=20,lwd=3.5)
points(x[2],y[2],pch=20,lwd=3.5)
points(x[3],y[3],pch=20,lwd=3.5)
points(x[4],y[4],pch=20,lwd=3.5)
points(x[5],y[5],pch=20,lwd=3.5)
abline(h=-8.5,v=-12.5)
xbar = mean(x)
ybar = mean(y)
segments(xbar,-8.5,xbar,6,lty=2)
text(xbar,-8.5,labels=expression(bar(x)),pos=1)
segments(-12.5,ybar,6,ybar,6,lty=2)
text(-12.5,ybar,labels=expression(bar(y)),pos=2)
...
text(-8,3,labels="SST",pos=2)
yhat = fitted.values(lm(y~x))[4]
arrows(-9,1,x[4],yhat,lty=4,col="blue",lwd=2)
arrows(-9,1,x[4],y[4],lty=4,col="blue",lwd=2)
text(-9,1,labels="SSE",pos=2)
text(-9,1,labels=expression(e[i]),pos=1)
arrows(5,1,x[4],yhat,lty=18,col="green",lwd=2)
arrows(5,1,x[4],ybar,lty=18,col="green",lwd=2)
text(5,1,labels="SSR",pos=4)
text(x[4],-8.5,labels=expression(x[i]),pos=1)
yhat1 = fitted.values(lm(y~x))[5]
arrows(0,5,x[5],yhat1,lwd=2)
text(0,5,labels=expression(paste(hat(y)," = ",b[0]+b[1]*x)),pos=3)
text(x[4],y[4],labels=expression(paste("(",x[i],",",y[i],")")),pos=4)
arrows(5,-5,x[4],yhat,lty=12,lwd=2)
text(5,-5,labels=expression(hat(y[i])),pos=1)
text(6,-8.5,labels="x",pos=1)
text(-12.5,6,labels="y",pos=2)
