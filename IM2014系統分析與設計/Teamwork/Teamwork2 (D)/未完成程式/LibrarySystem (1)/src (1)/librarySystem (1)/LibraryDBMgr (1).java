		...
		Connection conn = null;
			try {
					...
					else if(addTable.equals("paperbook"))
					{
						...
						main.setmaxID(rs.getInt("bookID"));
						pstmt.close();
						st.close();
					}
			...
			try {
					...
					else if(usefunction.equals("member"))//搜尋member
					{
						...
						if(searchList.isEmpty())
						{
							while(rs.next())
							{
								m1.setMember(rs.getString("memberID"), rs.getString("memberPassword"),rs.getString("email"),rs.getInt("numberofBorrowBook"),rs.getInt("numberOfOverdueBook"),rs.getInt("getnumberOfNoticeBook"),rs.getBoolean("right"));                            
							}
							System.out.println("搜尋成功");
							searchList.add(m1);
						}
